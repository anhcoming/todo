...
function updateCartCount(){ $$(".cart-count").forEach(e => e.textContent = Cart.count()); }
...
function renderHeader(){
  ...
  const ann = `<div class="announce"><div class="track">
  ...
  <header class="header"><div class="wrap header-row">
    ...
    <div class="icons">
      ...
      <a class="icon-btn" href="order.html" aria-label="Đơn hàng của tôi">${userI}</a>
...
function renderHome(){
  ...
  const hero = `<section class="hero">
    ...
    <div class="ticker"><div class="track">${Array(2).fill(`<span>★ FREESHIP 500K</span><span>★ SALE 50%</span><span>★ NEW ARRIVALS</span><span>★ ${S.BRAND.tagline.toUpperCase()}</span>`).join("")}</div></div>
  </section>`;
...
function renderCollection(){
  ...
  <div class="wrap shop">
    <aside class="filters" id="filters">
      <button class="close" id="fclose" style="display:none;font-size:24px;float:right">×</button>
  ...
  const fil=$("#filters");
  $("#ftoggle").onclick=()=>{fil.classList.add("open");$("#fclose").style.display="block"};
  $("#fclose").onclick=()=>fil.classList.remove("open");
  apply();
}
...
function renderProduct(){
  const root=$("#page");
  const p = S.getProduct(param("id")) || S.PRODUCTS[0];
  ...
  function draw(){
    ...
    const items = Cart.items.map((it,i)=>{
      ...
      return `<div class="cart-item">
        <a class="ci-thumb" href="product.html?id=${it.id}">${p?S.productSVG(p,{color:it.color}):""}</a>
        <div>
...
function openCheckout(){
  ...
  ov.innerHTML=`<div class="modal">
    ...
    <form id="ckForm" novalidate>
      <label class="fld"><span>Họ và tên *</span><input name="name" required></label>
      <label class="fld"><span>Số điện thoại *</span><input name="phone" required inputmode="tel"></label>
      <label class="fld"><span>Địa chỉ nhận hàng *</span><textarea name="address" rows="2" required></textarea></label>
  ...
  $("#ckForm").onsubmit=async(e)=>{
    ...
    const order={ customer_name:name, phone, address, note:f.note.value.trim(),
      items:Cart.items.map(x=>({id:x.id,name:x.name,price:x.price,color:x.color,size:x.size,qty:x.qty})),
...
async function renderOrder(){
  ...
  function lookupView(msg){
    ...
    root.innerHTML=`
    <div class="wrap page-head"><div class="crumb"><a href="index.html">Trang chủ</a> / Tra cứu đơn hàng</div>
      <h1 class="page-title">Tra cứu đơn hàng</h1></div>
    <div class="wrap" style="max-width:680px;padding-bottom:60px">
      ${msg?`<div class="notice err">${msg}</div>`:`<p class="muted" style="margin-bottom:16px">Nhập mã đơn (in trên màn hình sau khi đặt) để xem trạng thái giao hàng.</p>`}
      <form id="lookup" class="subscribe" style="max-width:none;margin:0 0 30px">
        <input id="codeInput" placeholder="Nhập mã đơn, ví dụ OR1A2B3" required>
        <button class="btn btn-dark" type="submit">Tra cứu</button>
      </form>
      ${recents.length?`<h3 style="font-family:var(--font-display);text-transform:uppercase;font-size:16px;margin-bottom:12px">Đơn gần đây trên thiết bị này</h3>
  ...
  if(!o){ lookupView(`Không tìm thấy đơn <strong>${escapeXML(code)}</strong>. Vui lòng kiểm tra lại mã.`); return; }
  ...
  const items=(o.items||[]).map(it=>{const pr=S.getProduct(it.id);
    return `<div class="cart-item">
      <div class="ci-thumb">${pr?productImage(pr,{color:it.color}):S.productSVG({type:"tee",print:it.name,collection:"",colors:[it.color||"#ccc"],name:it.name},{color:it.color})}</div>
