...
function renderLogin(user){
  ...
  document.body.innerHTML = `<div class="login-screen"><div class="login-card">
    ...
    <div class="login-note">${DB.cloud
      ? "Chỉ email admin đã cấu hình mới truy cập được."
      : "Đang ở <b>chế độ demo</b> (chưa cấu hình Supabase). Bấm nút trên để vào thử admin. Khi dán key Supabase, nút này sẽ là đăng nhập Google thật."}</div>
  </div></div>`;
  const lb=$("#loginBtn"); if(lb) lb.onclick=async()=>{ await DB.signInGoogle(); if(!DB.cloud) boot(); };
